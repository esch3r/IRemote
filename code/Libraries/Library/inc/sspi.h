...
#define SSP1_SET_PR(x)       LPC_SSP0->CPSR = (uint8)(SSP_CLK/x)-1
...
#define SSP_ENABLE_POWER(ch)  LPC_SC->PCONP |= PCONP_SSPI(ch)             //enable PCONP for SSPI
...
#define SSP0_SET_SCK          LPC_PINCON->PINSEL3  |= (0b11 << 08); LPC_PINCON->PINMODE3 |= (0b11 << 08)  // P1.20 pull-down Resistor
...
#define SSP_ENABLE_IRQ()      NVIC_EnableIRQ(SPI_IRQn)
#define SSP_SET_IRQS()       NVIC_SetPendingIRQ(SPI_IRQn)
...
#define SSP1_READ_CHAR(c)           LPC_SSP1->DR =  (c << 0)
